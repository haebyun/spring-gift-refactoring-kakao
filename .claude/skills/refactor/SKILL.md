...
- **body** (한국어): 아래 순서로 작성한다.
  1. **현재 문제** — 왜 변경이 필요한가 (첫 문장이 반드시 문제/이유를 서술해야 한다)
  2. **변경 방향** — 어떤 접근 방식으로 해결하는가
- **footer** (선택): 관련 이슈 번호 등

### 필수 포함 내용

- 작업 목록 단위로 커밋 메시지를 작성한다
- body의 첫 문장은 반드시 "왜 변경하는가"로 시작해야 한다. "무엇을 했는가"만 나열하지 않는다
...
```
refactor: MemberService 추출

- MemberController, AdminMemberController, KakaoAuthController의 비즈니스 로직을 MemberService로 이동
- 회원 등록/로그인, 카카오 로그인, 포인트 충전 등을 Service에서 담당
```

문제: "무엇을 이동했는가"만 나열하고, "왜 이동해야 하는가"가 없다.
